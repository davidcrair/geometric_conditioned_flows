...
class SplitConfig:
    ...
    Attributes:
        ...
        split_policy: holdout policy (strict_no_leakage, underrepresented_context)
    ...
    split_policy: str = "strict_no_leakage"
...
def apply_holdout_masks(adata: ad.AnnData, manifest: dict[str, Any]) -> dict[str, np.ndarray]:
    ...
    product_match = adata.obs[product_col].isin(selected_products).to_numpy(dtype=bool)

    policy = manifest.get("split_policy", "strict_no_leakage")
    if policy == "strict_no_leakage":
        is_held_out = is_pert_any & product_match
    elif policy == "underrepresented_context":
...
def validate_no_leakage(
    ...
    """validate that train and held-out sets are consistent
    ...
    Args:
        ...
        split_policy: split policy used to build masks
