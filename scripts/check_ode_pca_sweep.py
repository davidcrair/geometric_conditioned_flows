#!/usr/bin/env python
"""check status of ODE in PCA dimension sweep and generate train+predict commands"""
...
SBATCH_SCRIPT = "scripts/train_ode_pca_sweep.sbatch"
DEFAULT_PARTITION = "scavenge_gpu"


def _experiment_name(dim: int) -> str:
    return f"sciplex_ode_deg_pca_d{dim}"
...
def _sbatch_command(dim: int, partition: str) -> str:
    return (
        f"sbatch --partition={partition} "
        f"--export=ALL,DIM={dim} "
        f"{SBATCH_SCRIPT}"
    )
...
def check(
    generate: Annotated[
        bool, typer.Option("--generate", "-g", help="print local train+predict commands for missing models")
    ] = False,
    sbatch: Annotated[
        bool, typer.Option("--sbatch", "-b", help="print sbatch submissions for missing models")
    ] = False,
    partition: Annotated[
        str, typer.Option("--partition", "-p", help="slurm partition for --sbatch output")
    ] = DEFAULT_PARTITION,
    train: Annotated[
        bool, typer.Option("--train", "-t", help="train and predict all missing models sequentially")
    ] = False,
) -> None:
    """check which ODE PCA dimension sweep models are trained and predicted"""

    table = Table(
        title="ODE PCA Dimension Sweep Status",
        show_header=True,
        header_style="bold",
    )
    ...
    if sbatch:
        console.print(f"\n[bold]sbatch submissions (partition={partition}):[/]\n")
        for dim, train_cmd, predict_cmd in missing:
            if train_cmd:
                console.print(f"# train+predict d={dim}")
                console.print(_sbatch_command(dim, partition))
            elif predict_cmd:
                console.print(f"# predict d={dim} (checkpoint exists already)")
                console.print(predict_cmd)
            console.print()
    ...
    if train:
        ...
        for i, (dim, train_cmd, _) in enumerate(missing):
            ...
            if not _has_checkpoint(exp_name):
                console.rule(f"[bold][{i + 1}/{len(missing)}] train ODE PCA d={dim}[/]")
            ...
            if run_dir and _has_checkpoint(exp_name) and not _has_predictions(exp_name):
                console.rule(f"[bold][{i + 1}/{len(missing)}] predict ODE PCA d={dim}[/]")
