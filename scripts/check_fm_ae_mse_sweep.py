#!/usr/bin/env python
"""check status of FM in MSE AE latent space dimension sweep and generate train+predict commands"""
...
BASE_EXPERIMENT = "sciplex/fm_ae_latent_mse_d512"


def _experiment_name(dim: int) -> str:
    return f"sciplex_fm_deg_ae_mse_d{dim}"


def _ae_artifact_tag(dim: int) -> str:
    return f"sciplex_ae_deg_mse_d{dim}"


def _has_ae_projection(dim: int) -> bool:
    tag = _ae_artifact_tag(dim)
...
def _collect_missing() -> list[tuple[int, str | None, str | None]]:
    """collect missing (dim, train_cmd_or_none, predict_cmd_or_none) tuples"""

    missing = []
    for dim in DIMS:
        exp_name = _experiment_name(dim)
        if not _has_predictions(exp_name):
            if not _has_ae_projection(dim):
                missing.append((dim, None, None))
                continue
...
def check(
    ...
    """check which FM MSE AE dimension sweep models are trained and predicted"""

    table = Table(
        title="FM in MSE AE Latent Space Dimension Sweep Status",
        show_header=True,
        header_style="bold",
    )
    ...
    for dim in DIMS:
        ...
        table.add_row(str(dim), ae_str, run_str, ckpt_str, pred_str)
    ...
    if generate:
        ...
        for dim, train_cmd, predict_cmd in missing:
            if train_cmd is None and predict_cmd is None:
                console.print(f"# d={dim}: [red]missing AE projection[/] -- train MSE AE first")
                console.print()
                continue
            if train_cmd:
    ...
    if train:
        ...
        for i, (dim, train_cmd, _) in enumerate(missing):
            ...
            if not _has_ae_projection(dim):
                console.print(f"[red]SKIP d={dim}: missing AE projection[/]")
                continue

            # train if needed
            if not _has_checkpoint(exp_name):
                console.rule(f"[bold][{i + 1}/{len(missing)}] train FM MSE AE d={dim}[/]")
            ...
            if run_dir and _has_checkpoint(exp_name) and not _has_predictions(exp_name):
                console.rule(f"[bold][{i + 1}/{len(missing)}] predict FM MSE AE d={dim}[/]")
